test_that("integration test for CRAN", {
  withr::with_tempdir({
    ...
    expect_equal(report_code[155], "vt_file(vt_path(child_files),dynamic_referencing = TRUE)")
...
test_that("validation report in package",{
  ...
  withr::with_tempdir({
    ...
    expect_equal(report_code[3], paste0("author: ", test_user))
    expect_equal(report_code[27], paste0("  library(", basename(getwd()), ")"))
    expect_equal(report_code[155], "vt_file(vt_path(child_files),dynamic_referencing = FALSE)")
