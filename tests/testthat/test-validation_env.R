...
test_that("temp", {
  ...
  # bare
  validation_env0 <- vt_scrape_val_env(temp_path)
  ...
  writeLines(con = normalizePath(file.path(temp_path, "DESCRIPTION")),
             text = desc_contents$str(by_field = TRUE, normalize = FALSE, mode = "file"))
  validation_env1 <- vt_scrape_val_env(temp_path)
  ...
  validation_env2 <- vt_scrape_val_env(temp_path)
  ...
  validation_env3 <- vt_scrape_val_env(temp_path)
  ...
  testthat::expect_error(vt_scrape_val_env(temp_path),
