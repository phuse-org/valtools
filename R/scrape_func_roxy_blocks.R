#' @title Retrieve the value in a function specification block of a custom section
  ...
  do.call(rbind, lapply(seq_along(refs), function(i) {
    ...
    if (length(out_cleaned) == 0 | length(grep(out_cleaned, pattern = "#' @section")) == 0) {
      ...
      return(data.frame(title = as.character(parse(text = as.character(refs[[i]]))[[1]])[2],
                        ...
                        last_updated_date = lubridate::parse_date_time(get_section_contents("@section last updated* date:", tolower(roxy_block)),
                                                                       orders = c("ymd", "mdy")),
                        stringsAsFactors = FALSE))
    }
  }))
