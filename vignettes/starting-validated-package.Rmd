...
In general, the process consists of the following steps, and the
vignette as specific examples for each.
...
To check out the folder structure of the new package, run the following
function:
...
`vignettes/validation/requirements` folder by default.
...
Ultimately, the purpose of validation is to generate documentation that
the package meets the goals. {valtools} supports this generation of
documentation by creating a final report that can run and gather all the
information that was generated in the prior steps to create the final
report.

The report is written in Rmarkdown so that it is flexible and dynamic.
{valtools} provides a function to create a template that can be worked
off of to generate the report: `vt_use_report()`.

`vt_use_report()` initiates the validation report rmarkdown file using a template
provided via {valtools} in the working directory identified
in the validation config file. Within packages this defaults to the `vignettes` folder.
...
-   Validation: record each requirement, testcase and results of the
    test code
...
Run each method of validation and observe the diffeences
