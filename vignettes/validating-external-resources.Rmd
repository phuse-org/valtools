...
vt_create_packet(path = "example_validation_packet", target = "valtools")

```

The validation packet folder is constructed using the `usethis::create_project()` function, and then all the necessary validation infrastructure required for
{valtools}.
...
Run the chunk of code below to create a validation packet in a temporary directory
to follow along with the tutorial for the {whoami} package. The new packet project will be opened new session. Run all subsequent code in that new session.
...
editor, editDate, and riskAssessment. The last the roxygen comments are custeom
...
is directly tied to validation for recording changes. The purpose of this is to capture update and information that is useful for developers from
